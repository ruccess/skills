...
결정 트리의 모든 가지를 질문으로 하나씩 해소한다 — 각 질문에 추천 답을 붙이고, 코드가 답할 수 있는 건 묻지 않고 읽는다. 용어는 프로젝트 용어집(CONTEXT.md)과 대조하고, 해소된 결정은 바로 문서에 남긴다. ADR은 되돌리기 어렵고·맥락 없이 놀랍고·진짜 트레이드오프일 때만 작성한다. 세션이 만진 한국어 문서는 마지막에 korean-docs 파이프라인으로 다듬는다. grill-with-docs를 증류한 경량판이다.
...
일곱 단계를 순서대로 통과해야 PR이 나간다. 게이트가 막히면 고친 뒤 2단계부터 다시 탄다 — 이후 게이트가 수정분을 봐야 하기 때문이다.
