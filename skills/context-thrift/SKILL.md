...
Measured on one of the author's real sessions (reproduce yours with `bunx ccusage`): 855 tool calls spread across 854 messages (parallelism 1.00) — the round-trips alone burned 400M cache-read tokens, 62% of the session's cost. Batching the same calls three-per-message cuts that by two thirds.
